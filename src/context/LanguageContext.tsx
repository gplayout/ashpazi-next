...
export function LanguageProvider({ children }: { children: ReactNode }) {
    ...
    const t = (obj: Record<string, unknown>, key: string): unknown => {
        if (!obj) return '';
        ...
        if (obj.nutrition_info) {
            ...
            const nutritionInfo = obj.nutrition_info as Record<string, Record<string, unknown>>;
            const node =
                nutritionInfo[langCode] ||
                (langCode === 'en'
                    ? nutritionInfo.english
                    : langCode === 'fa'
                      ? nutritionInfo.persian
                      : langCode === 'es'
                        ? nutritionInfo.spanish
                        : null);
        ...
        if (obj.recipe_translations && Array.isArray(obj.recipe_translations)) {
            const translation = (obj.recipe_translations as Record<string, unknown>[]).find(
                tr => tr.language_code === language
            );
            if (translation) {
        ...
        if (language !== 'en') {
            ...
            if (obj.recipe_translations && Array.isArray(obj.recipe_translations)) {
                const enTrans = (obj.recipe_translations as Record<string, unknown>[]).find(
