...
interface InstagramProfile {
    handle: string;
    last_posts: InstagramPost[];
}

interface SkuItem {
    title: string;
    original_caption_snippet: string;
    description: string;
    price_detected: string | null;
...
export class SkuAgent {
     ...
     */
    async generateSkus(profile: InstagramProfile, language = 'en'): Promise<SkuItem[]> {
        const postsText = profile.last_posts
            .map((p, i) => `Post ${i + 1}: "${p.caption}" (Image: ${p.image_url})`)
            .join('\n');
