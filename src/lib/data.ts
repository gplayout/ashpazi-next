...
import type { RecipeProps, NutritionLangData } from '@/types';
...
export async function getRecipeBySlug(
    ...
    try {
        ...
        if (lang !== 'en') {
            console.log(
                `[DataLayer] Tier 2: Attempting Raw Translation Fetch for ${uuid} (${lang})`
            );
            ...
            if (rawTrans) {
                ...
                if (!spineErr && spineBase) {
                    ...
                    const content = rawTrans as Record<string, any>; // Flattened Table

                    if (content) {
                        // STRICT NO ENGLISH BLEED
                        // 1. Ingredients: Do NOT backfill from Spine (EN)
                        // Content is likely { uuid: { label: "" } }
                        const ings: string[] = content.ingredients
                            ? Object.values(content.ingredients as Record<string, any>).map(
                                  (i: any) => i.label || i.text
                              )
                            : [];

                        // 2. Instructions: Do NOT backfill from Spine (EN)
                        // Column is 'instructions' usually holding step map
                        const steps: string[] = content.instructions
                            ? Object.values(content.instructions as Record<string, any>)
                            : content.steps
                              ? Object.values(content.steps as Record<string, any>)
                              : [];
                        ...
                        if (ings.length === 0 || steps.length === 0) {
                            ...
                            try {
                                ...
                                fs.appendFileSync(
                                    'logs/data_gaps.log',
                                    `[${new Date().toISOString()}] PARTIAL_TRANS|${uuid}|${lang}|Ings:${ings.length}|Steps:${steps.length}\n`
                                );
                            } catch (e) {}
                            console.warn(
                                `[DataLayer] Partial Translation for ${uuid} (${lang}). Rendering empty sections to prevent bleed.`
                            );
                        }

                        const nutritionInfoRaw: Record<string, NutritionLangData> = {
                            [lang]: {
                                name: content.title || '',
                                ...
                                times: {
                                    prep:
                                        (spineBase as any).recipe_metadata?.[0]
                                            ?.prep_time_minutes || 0,
                                    cook:
                                        (spineBase as any).recipe_metadata?.[0]
                                            ?.cook_time_minutes || 0,
                                ...
                                difficulty_level:
                                    (spineBase as any).recipe_metadata?.[0]?.difficulty || 'Medium',
                                category:
                                    (spineBase as any).recipe_metadata?.[0]?.category || 'Global',
                                nutrition: { calories: 0, protein: '0g', carbs: '0g', fat: '0g' },
                            },
                        };

                        return {
                            id: (spineBase as any).id,
                            legacy_id: (spineBase as any).legacy_recipe_id,
                            name: content.title || '', // No fallback to EN title
                            description: content.description || '',
                            instructions: steps,
                            ingredients: ings,
                            prep_time_minutes: (spineBase as any).recipe_metadata?.[0]
                                ?.prep_time_minutes,
                            cook_time_minutes: (spineBase as any).recipe_metadata?.[0]
                                ?.cook_time_minutes,
                            difficulty: (spineBase as any).recipe_metadata?.[0]?.difficulty,
        ...
        if ((spine as any).recipe_metadata && (spine as any).recipe_metadata.length > 0) {
        ...
        const enMeta: Record<string, any> =
            (spine as any).content_translations?.find((t: any) => t.language_code === 'en') || {};

        // 1. Parse QA Metadata (Rich Content) FIRST
        const qa: Record<string, any> = enMeta.qa_metadata || {};

        // Resolve Category (Priority: QA -> DB Meta -> Default)
        const displayCategory: string =
            qa.category ||
            (spine as any).recipe_metadata?.[0]?.cuisine ||
            (spine as any).recipe_metadata?.[0]?.category ||
            'Global';

        // FORCE LOCAL IMAGE (Governance)
        const imageSrc: string = `/recipe-images/${uuid}.jpg`;

        // 2. Construct Nutrition Info Object with Rich Data
        const nutritionInfo: Record<string, NutritionLangData> = {
            en: {
                name: enMeta.title || 'Untitled Recipe',
                description: enMeta.description || '',
                ingredients: (spine as any).recipe_ingredients.map((ing: any) =>
                    ing.note_text
                        ? `${ing.label_text || ing.raw_text}, ${ing.note_text}`
                        : ing.label_text || ing.raw_text
                ),
                instructions: (spine as any).recipe_steps.map(
                    (s: any) => s.instruction_text || s.instruction
                ),
                times: {
                    prep: (spine as any).recipe_metadata?.[0]?.prep_time_minutes || 30,
                    cook: (spine as any).recipe_metadata?.[0]?.cook_time_minutes || 45,
                },
                difficulty_level: (spine as any).recipe_metadata?.[0]?.difficulty || 'Medium',
        ...
        try {
            ...
            if (!fs.existsSync('logs')) fs.mkdirSync('logs');
            fs.appendFileSync(
                'logs/data_access.log',
                `[${new Date().toISOString()}] FETCH|${slug}|${lang}|${compiled ? 'COMPILED' : 'FALLBACK'}|${uuid}\n`
            );
        } catch (e) {}
        ...
        return {
            id: (spine as any).id,
            legacy_id: (spine as any).legacy_recipe_id,
            name: enMeta.title || 'Untitled Recipe',
            description: enMeta.description || '',
            // Map Metadata
            prep_time_minutes: (spine as any).recipe_metadata?.[0]?.prep_time_minutes || 30,
            cook_time_minutes: (spine as any).recipe_metadata?.[0]?.cook_time_minutes || 45,
            difficulty: (spine as any).recipe_metadata?.[0]?.difficulty || 'Medium',
            category: displayCategory,
            ...
            ingredients: (spine as any).recipe_ingredients.map((ing: any) => {
                const raw: string = ing.label_text || ing.raw_text || '';
                const note: string | undefined = ing.note_text;
...
function normalizeCompiled(json: Record<string, any>, id: string, lang: string): RecipeProps {
    const stepsArray: any[] = Array.isArray(json.instructions)
        ? json.instructions
        : Array.isArray(json.steps)
          ? json.steps
          : [];
    const ingsArray: any[] = Array.isArray(json.ingredients) ? json.ingredients : [];

    const normInstructions: string[] = stepsArray
        .map((s: any) => {
            if (typeof s === 'string') {
    ...
    const normIngredients: string[] = ingsArray.map((i: any) => {
        const label: string = i.label || i.text || '';
        const note: string | undefined = i.note;
