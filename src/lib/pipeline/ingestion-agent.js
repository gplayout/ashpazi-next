import { pipelineClient as supabase } from '../pipeline-client.js';
import { ERROR_CODES } from './state-machine.js';

// --- Cache State ---
let cachedIngKeys = null; // Array of names sorted by length desc
let cachedIngMap = null;  // Map: Name -> ID
let cachedUnitKeys = null;
...
async function loadDictionaries() {
    ...
    lastCacheTime = now;
    console.log(`[Pipeline] Dictionaries refreshed internally. ${cachedIngKeys.length} ings, ${cachedUnitKeys.length} units.`);
}
...
export class IngestionAgent {
     ...
     */
    static async process(recipeStateRow) {
        try {
            // 1. Load Data
            ...
            // 2. Parse Ingredients
            const rawIngredients = legacyRecipe.ingredients || [];
            ...
            for (const rawLine of rawIngredients) {
                ...
                mappedIngredients.push({
                    ...
                    unit_id: foundUnitName ? cachedUnitMap.get(foundUnitName) : null,
            ...
            const { data: groupRes, error: groupErr } = await supabase
                ...
                .upsert({
                    ...
                    slug: 'main',
                    display_order: 0
                }, { onConflict: 'recipe_id, slug' }) // Assuming generic constraint/logic
                .select('id')
                .single();
            ...
            if (groupErr || !groupId) {
                ...
                const { data: g2 } = await supabase.from('recipe_groups').select('id').eq('recipe_id', newRecipeId).eq('slug', 'main').single();
                if (g2) groupId = g2.id;
                else {
                    const { data: g3, error: g3Err } = await supabase.from('recipe_groups').insert({ recipe_id: newRecipeId, slug: 'main', display_order: 0 }).select('id').single();
