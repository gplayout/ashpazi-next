...
function StoryContent({ story }: { story: import('@/types').FeedRecipe }) {
    const { language, t } = useLanguage();

    // AI/Chef Zaffaron Data Extraction
    // FIX: Use current language, fallback to EN if specific translation missing
    const translation =
        story.recipe_translations?.find(tr => tr.language_code === language) ||
        story.recipe_translations?.find(tr => tr.language_code === 'en');

    const metadata = translation?.qa_metadata;
    const isChefPick = ((metadata?.internal_score as any)?.marketing_joy_score || 0) > 85;

    // Prefer AI Title/Desc -> Default Translation -> Raw
    const displayName = String(translation?.title || t(story as Record<string, unknown>, 'name'));
    const displayDescription = String(
        metadata?.marketing_description ||
            t(story as Record<string, unknown>, 'description') ||
            story.intro
    );

    return (
        <div
            className="absolute bottom-0 left-0 right-0 p-6 flex flex-col gap-4 text-white"
...
export default function Stories({ recipes = [] }: { recipes?: import('@/types').FeedRecipe[] }) {
    const { language, t } = useLanguage();
    ...
    const markAsSeen = (id: string): void => {
    ...
    const validStories = recipes
        ...
        .sort((a, b) => {
            ...
            const scoreA =
                (a.recipe_translations?.[0]?.qa_metadata?.internal_score as any)
                    ?.marketing_joy_score || 0;
            const scoreB =
                (b.recipe_translations?.[0]?.qa_metadata?.internal_score as any)
                    ?.marketing_joy_score || 0;
    ...
    const handleStoryClick = (idx: number, recipeId: string): void => {
        setViewingIndex(idx);
        markAsSeen(recipeId);
    };
                    ...
                    {displayStories.map((recipe, idx) => {
                        ...
                        return (
                            ...
                            >
                                    ...
                                    >
                                        <div className="p-[2px] rounded-full bg-background">
                                            <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden">
                                                <Image
                                                    src={recipe.image}
                                                    alt={String(recipe.name_en || recipe.name)}
                                                    fill
                                ...
                                {/* Label */}
                                <span
                                    className={`text-xs font-medium w-20 text-center truncate ${
                                        isSeen
                                            ? 'text-zinc-400 dark:text-zinc-500'
                                            : 'text-foreground/80'
                                    }`}
                                >
                                    {String(t(recipe as Record<string, unknown>, 'name'))}
                                </span>
                            </button>
                        );
                    })}
...
function StoryViewerPortal({ children }: { children: React.ReactNode }) {
