...
export default function SocialShareModal({ isOpen, onClose, recipe }) {
    ...
    // Data Extraction
    const translation = recipe?.recipe_translations?.find(tr => tr.language_code === (language === 'fa' ? 'fa' : 'en'));
    ...
    const calories = recipe.nutrition_info?.[language]?.calories || recipe.nutrition_info?.caloric_balance?.calories || 'Healthy';
    const displayCalories = language === 'fa' ? toPersianDigits(calories) : calories;
    const prepTime = language === 'fa' ? toPersianDigits(recipe.prep_time_minutes) : recipe.prep_time_minutes;
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" dir={language === 'fa' ? 'rtl' : 'ltr'}>
            <div className="bg-background rounded-3xl w-full max-w-md overflow-hidden relative border border-border">
                ...
                <div className="p-4 flex flex-col items-center gap-4">
                    ...
                    >
                        ...
                        {/* Text Overlay */}
                        <div className={`absolute bottom-0 left-0 right-0 p-8 text-white flex flex-col ${language === 'fa' ? 'items-end text-right' : 'items-center text-center'}`}>

                            <div className="mb-2 uppercase tracking-widest text-xs font-bold text-amber-500">
                                {language === 'fa' ? 'دستپخت من' : 'I Cooked This'}
                            </div>

                            <h2 className={`text-2xl font-black leading-tight mb-4 ${language === 'fa' ? 'font-farsi' : ''}`} style={{ textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>
                                {displayName}
                            </h2>

                            {/* Stats */}
                            <div className="flex gap-4 mb-8 text-sm font-medium opacity-90" dir="ltr">
                                <span>⏱️ {prepTime}{language === 'fa' ? ' م' : 'm'}</span>
                    ...
                    <div className="flex gap-3 w-full">
                        ...
                        >
                            {isGenerating ? 'Generating...' : (
                                <>
                                    <Instagram size={20} />
                                    <span>{language === 'fa' ? 'دانلود استوری' : 'Download Story'}</span>
                                </>
                            )}
                        </button>
                    </div>
