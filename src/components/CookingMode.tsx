...
const CookingMode = ({
    ...
    // Parse instructions
    const rawInstructions = t(recipe as unknown as Record<string, unknown>, 'instructions') || [];
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black text-white flex flex-col animate-in fade-in duration-300">
            ...
            {/* Header */}
            <div
                className={`absolute top-0 left-0 right-0 p-4 flex justify-between items-center transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'}`}
            >
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-8">
                <div className="max-w-2xl bg-black/20 p-6 rounded-3xl backdrop-blur-sm">
                    <h2
                        className="text-2xl md:text-4xl font-bold leading-relaxed tracking-tight"
            ...
            {/* Controls */}
            <div
                className={`absolute bottom-0 left-0 right-0 p-8 flex justify-between items-center transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'}`}
            >
                <button
                    onClick={handlePrev}
