...
const RecipeCard = ({
    ...
    recipe: import('@/types').FeedRecipe;
    priority?: boolean;
}) => {
    ...
    const slugSource = (recipe.name_en as string) || recipe.name;
    ...
    const slug =
        ...
        recipe.id ||
        recipe.slug ||
        slugSource.toLowerCase().replace(/\s+/g, '-');

    // Localized fields
    // Hard Guard: Fallback to legacy 'name' if translation fails
    const displayName = String(t(recipe as Record<string, unknown>, 'name') || recipe.name);
    // --- Description Resolution Logic (Safe & Smart) ---
    const translations = recipe.recipe_translations || [];

    // Helper: Detect Farsi
    const isMostlyFarsi = (text: string | null | undefined): boolean => {
    ...
    const extractText = (field: unknown): string[] => {
        ...
        return raw
            .map(step => (typeof step === 'object' && step?.text ? step.text : step))
    ...
    let finalDescription =
        targetTr?.qa_metadata?.marketing_description ||
    ...
    if (!finalDescription && language !== 'fa') {
        ...
        if (enTr) {
            finalDescription =
                enTr.qa_metadata?.marketing_description ||
            ...
            <Link href={`/recipe/${slug}?id=${recipe.id}`} className="block h-full">
                <Card className="overflow-hidden border border-border/40 bg-card hover:shadow-lg transition-all duration-300 hover:-translate-y-1 h-full flex flex-col">
                    ...
                    <div className="relative aspect-[4/3] overflow-hidden w-full bg-muted">
                        ...
                        <Badge
                            variant="secondary"
                            className="absolute top-3 left-3 backdrop-blur-md bg-white/70 dark:bg-black/50 text-xs font-medium z-10"
                        >
                    ...
                    <CardHeader className="p-4 pb-2">
                        <CardTitle
                            className={`text-lg font-bold leading-tight line-clamp-2 group-hover:text-primary transition-colors ${language === 'fa' ? 'font-vazirmatn text-right' : 'font-outfit text-left'}`}
                        >
                            {displayName}
                        </CardTitle>
                    </CardHeader>
                    ...
                    <CardContent className="p-4 pt-1 flex-grow">
                        <p
                            className={`text-sm text-muted-foreground line-clamp-2 ${language === 'fa' ? 'text-right' : 'text-left'}`}
                        >
                            {finalDescription}
                        </p>
                    </CardContent>
                    ...
                    <CardFooter
                        className={`p-4 pt-0 flex justify-between items-center text-xs text-muted-foreground border-t border-border/40 mt-auto bg-muted/20 ${language === 'fa' ? 'flex-row-reverse' : ''}`}
                    >
                        <div className="flex items-center gap-1">
                            <Clock size={14} className="text-amber-600" />
                            <span>
                                {displayTime} {getUiLabel('minutes', language)}
                            </span>
                        </div>
                        <div className="flex items-center gap-1">
