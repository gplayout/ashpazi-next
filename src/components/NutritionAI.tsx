...
export default function NutritionAI({
    ...
    recipe: { name?: string; title?: string; ingredients?: string[] };
    ...
    const effectiveLoading =
        isLoading !== undefined ? isLoading : initialData ? false : internalLoading;
                ...
                <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-xl shadow-emerald-500/5 relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {[
                            ...
                            <div
                                key={macro.label}
                                className="bg-zinc-50 dark:bg-zinc-800/50 p-2 rounded-xl text-center"
                            >
                                <p className="text-[10px] uppercase tracking-wider text-zinc-500 mb-1">
                                    {getUiLabel(macro.label, language)}
                                </p>
                                <p className="font-bold text-base">{macro.value || '-'}</p>
                                <div
                                    className={`w-full h-1 mt-1 rounded-full opacity-50 ${macro.color}`}
                                ></div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-4">
                        {data.diet_labels?.map((label: string) => (
                            <Badge
                                key={label}
                                variant="secondary"
                                className="text-xs bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300"
                            >
                                {label}
                            </Badge>
                        ))}
                ...
                {data.health_benefits && (
                    <div className="bg-white dark:bg-zinc-900 p-5 rounded-2xl border border-zinc-100 dark:border-zinc-800">
                        ...
                        <ul className="space-y-2">
                            {data.health_benefits.map((benefit: string, idx: number) => (
                                <li
                                    key={idx}
