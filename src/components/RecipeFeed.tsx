...
export default function RecipeFeed({
    ...
    initialRecipes: import('@/types').FeedRecipe[];
}) {
    const { language } = useLanguage();
    const [recipes, setRecipes] = useState(initialRecipes as import('@/types').FeedRecipe[]);
    ...
    return (
        <>
            <div
                className={`flex items-center justify-between mb-8 ${language === 'fa' ? 'flex-row-reverse' : ''}`}
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                {loading &&
                    [...Array(4)].map((_, i) => <RecipeCardSkeleton key={`skeleton-${i}`} />)}
            </div>
            ...
            <div ref={observerTarget} className="w-full flex justify-center py-12">
                ...
                {!hasMore && (
                    <p className="text-muted-foreground text-sm">
                        {getUiLabel('reached_end', language)}
                    </p>
                )}
            </div>
        </>
