...
export async function POST(request: NextRequest) {
    ...
    try {
        const {
            ...
            recipeContext,
            language = 'en',
        }: AssistantRequestBody = await request.json();
        ...
        return NextResponse.json(
            {
                text: `My apologies, the kitchen is a bit busy right now! (System Error: ${error instanceof Error ? error.message : 'Unknown error'})`,
            },
            { status: 500 }
        );
    }
