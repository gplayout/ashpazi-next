    ...
    try {
        ...
        if (!rows || rows.length === 0) {
            return NextResponse.json({
                ok: true,
                summary: { processed: 0, normalized_ok: 0, blocked_review: 0, message: 'No pending recipes' }
            });
        }
        ...
        const results = await Promise.all(rows.map(async (row) => {
            ...
            const { error: startErr } = await pipelineClient
                .from('recipe_pipeline_state')
                .update({ status: 'ingestion_in_progress', last_processed_at: new Date().toISOString() })
            ...
            if (result.success) {
                ...
                await pipelineClient
                    ...
                    .eq('legacy_recipe_id', row.legacy_recipe_id);
                return { id: row.legacy_recipe_id, status: 'blocked_review', error: result.error };
            }
        }));
