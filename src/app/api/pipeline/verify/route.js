...
export async function GET(request) {
    ...
    try {
        ...
        if (mode === 'check') {
            const { data } = await supabase.from('recipe_pipeline_state')
                .select('legacy_recipe_id, status, error_log')
                .in('legacy_recipe_id', [TEST_ID_VALID, TEST_ID_INVALID]);

            // Check Registry for Valid ID
            const { data: reg } = await supabase.from('registry_recipes').select('id, legacy_recipe_id').eq('legacy_recipe_id', TEST_ID_VALID);
        ...
        if (mode === 'cleanup') {
            ...
            const { data: regs } = await supabase.from('registry_recipes').select('id').in('legacy_recipe_id', [TEST_ID_VALID, TEST_ID_INVALID]);
            ...
            await supabase.from('recipe_pipeline_state').delete().in('legacy_recipe_id', [TEST_ID_VALID, TEST_ID_INVALID]);
            await supabase.from('recipes').delete().in('id', [TEST_ID_VALID, TEST_ID_INVALID]);

            return NextResponse.json({ message: 'Cleanup Done' });
        }

        return NextResponse.json({ message: 'Usage: ?mode=[setup|run-agent|trigger-api|check|cleanup]' });
