...
export async function GET(request: NextRequest) {
    try {
        ...
        console.log('Auth Check:', {
            incoming: secret,
            env: process.env.NEXT_PUBLIC_PIPELINE_SECRET ? 'Exits' : 'Missing',
        });
        ...
        let rows: { legacy_recipe_id: number; status: string }[] = [];
        ...
        for (const row of rows) {
            ...
            try {
                ...
                if (!isRetry) {
                    ...
                    if (existing) {
                        console.log(
                            `[Skip] LegacyId: ${legacyId} -> UUID: ${recipeUUID} has translation ${existing.id} (${targetLang})`
                        );
                        results.skipped++;
                        results.details.push({
                            id: legacyId,
                            status: 'skipped',
                            reason: `Translation exists: ${existing.id}`,
                        }); // Debug info
                        continue;
                    }
                }
                ...
                const agentInput = {
                    ...
                    source_instructions: Array.isArray(sourceData.instructions)
                ...
                console.log(
                    `[Translate Debug] Context for ${sourceData.name}:`,
                ...
                const quality = QualityScorer.score({
                    ...
                    targetText: translation.title as string,
                    sourceStepCount: agentInput.source_instructions.length,
                    targetStepCount: (translation.instructions as unknown[]).length,
                    glossaryTokens: [],
                });

                // 7. Save
                const { error: saveErr } = await pipelineClient.from('content_translations').upsert(
                    {
                        recipe_id: recipeUUID,
                ...
                if (translation.category) {
                    ...
                    if (legacyUpdateErr) {
                        console.error(
                            `Failed to update legacy category for ${legacyId}:`,
                    ...
                    if (!verifyData || verifyData.category !== translation.category) {
                        ...
                        throw new Error(
                            `CRITICAL: Data Verification Failed! Expected category '${translation.category}' but found '${actual}'. Write operation may have been silently ignored.`
                        );
                    }

                    console.log(
                        `[Root Fix] Verified recipes.category for ${legacyId} is now "${translation.category}"`
                    );
                }
                ...
                if (stateErr)
                    console.warn(`State update warning for ${legacyId}:`, stateErr.message);
