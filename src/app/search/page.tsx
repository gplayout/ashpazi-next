...
export default function SearchPage() {
    ...
    const performSearch = async () => {
        ...
        try {
            ...
            if (filters.timeRange !== null) params.set('time', String(filters.timeRange));
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {DIFFICULTY_OPTIONS.map(opt => (
                                        <Badge
                                            key={opt}
                                            variant={
                                                filters.difficulty === opt ? 'default' : 'outline'
                                            }
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {TIME_OPTIONS.map((opt, idx) => (
                                        <Badge
                                            key={idx}
                                            variant={
                                                filters.timeRange === idx ? 'default' : 'outline'
                                            }
                                            className="cursor-pointer hover:bg-primary/20"
                                            onClick={() =>
                                                setFilters(f => ({
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {CATEGORY_OPTIONS.map(opt => (
                                        <Badge
                                            key={opt}
                                            variant={
                                                filters.category === opt ? 'default' : 'outline'
                                            }
                                            className="cursor-pointer hover:bg-primary/20"
                                            onClick={() =>
                                                setFilters(f => ({
                    ...
                    <div className="text-center py-20">
                        {query.length >= 2 || hasActiveFilters ? (
                            <p className="text-muted-foreground">
                                {getUiLabel('no_results', language)}
                            </p>
                        ) : (
                            <div className="space-y-4">
                                <Search size={48} className="mx-auto text-muted-foreground/30" />
                                <p className="text-muted-foreground">
                                    {getUiLabel('start_typing', language)}
                                </p>
                            </div>
                        )}
                    </div>
