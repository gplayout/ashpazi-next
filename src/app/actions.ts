...
if (!supabaseServiceRoleKey) {
    console.error('FATAL: SUPABASE_SERVICE_ROLE_KEY is missing in actions.ts!');
}
 ...
 */
async function fetchRecipesManifest(
    page: number,
    limit: number,
    lang: string
): Promise<FeedRecipe[]> {
    ...
    return (data as ManifestRow[]).map(row => ({
 ...
 */
async function fetchRecipesLegacy(page: number, limit: number): Promise<FeedRecipe[]> {
    const from = (page - 1) * limit;
    const to = from + limit - 1;
    ...
    const legacyIds = (recipes as LegacyRecipe[]).map(r => r.id);
    ...
    // 3. Simple Enrichment
    const enriched = (recipes as LegacyRecipe[]).map(r => {
        const reg = (registryMap as RegistryMapRow[] | null)?.find(
            jm => jm.legacy_recipe_id === r.id
        );
        return {
            ...r,
    ...
    return enriched as unknown as FeedRecipe[];
}
