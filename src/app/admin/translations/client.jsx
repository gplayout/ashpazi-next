...
export default function TranslationReviewClient({ drafts, initialStats }) {
    ...
    async function handleBatchTranslation(continuous = false) {
        ...
        async function step() {
            ...
            try {
                ...
                const res = await fetch(`/api/pipeline/translate?secret=${secret}&lang=${selectedLang}&offset=${currentOffset}`);

                if (!res.ok) {
                    // If error, maybe skip simple batch
                    console.error("Batch failed, skipping...");
                    setCurrentAction(`Error ${res.status}. Retrying offset ${currentOffset + 5}...`);
                    currentOffset += 5;
                    if (continuous && !stopRef.current) setTimeout(step, 2000);
                    else { setIsBatching(false); setIsAuto(false); setIsStopping(false); }
                    return;
                }
                ...
                if (data.ok) {
                    ...
                    if (data.message === 'No normalized recipes found' || (currentOffset > 3500)) { // Increased limit
                        alert('All caught up! No more recipes to translate (or end of DB reached).');
                    ...
                    if (continuous) {
                        ...
                        setCurrentAction(`Batch ok: ${summary.success} success, ${summary.skipped} skipped. Next...`);
                    }
                ...
                currentOffset += 5;
                if (continuous) setTimeout(step, 2000); // Retry logic
                else { setIsBatching(false); setIsAuto(false); }
            }
        }
    ...
    return (
        <div style={{ padding: '20px' }}>
            {/* Batch Controls */}
            <div style={{ padding: '20px', background: '#f5f5f5', marginBottom: '30px', borderRadius: '8px', border: '1px solid #ddd' }}>
                <h2 style={{ marginTop: 0 }}>Batch Translation Tool</h2>
                {initialStats && (
                    <div style={{ marginBottom: '15px', padding: '10px', background: '#e0f2fe', borderRadius: '4px' }}>
                        <strong>Database Total: {initialStats.total} translations.</strong>
                        <div style={{ fontSize: '0.9em', color: '#555', marginTop: '5px' }}>
                            {Object.entries(initialStats.byLang).map(([k, v]) => <span key={k} style={{ marginRight: '10px' }}>{k.toUpperCase()}: {v}</span>)}
                        </div>
                    </div>
                )}
                <p style={{ marginBottom: '15px' }}>Generate translations for missing content in the selected language.</p>
                ...
                {isAuto && (
                    <div style={{ marginTop: '15px', padding: '10px', background: '#fff', border: '1px solid #ddd', borderRadius: '4px' }}>
                        ...
                        <span style={{ marginLeft: '15px', color: 'green' }}>Success: {totalStats.success}</span>
                        <span style={{ marginLeft: '15px', color: '#d97706' }}>Skipped: {totalStats.skipped}</span>
                        <span style={{ marginLeft: '15px', color: 'red' }}>Failed: {totalStats.failed}</span>
                        <div style={{ marginTop: '10px', fontSize: '14px', color: '#555', fontFamily: 'monospace' }}>
                ...
                {/* Last Batch Result */}
                {batchResult && !isAuto && (
                    <div style={{ marginTop: '20px', padding: '15px', background: '#e0f2fe', borderRadius: '4px', border: '1px solid #bae6fd' }}>
                            ...
                            <tr key={item.id} style={{ borderBottom: '1px solid #ddd' }}>
                                ...
                                <td style={{ padding: '10px' }}>
                                    <span style={{
                                        padding: '2px 6px',
                                        borderRadius: '4px',
                                        background: item.publish_status === 'published' ? '#dcfce7' : '#fef9c3',
                                        color: item.publish_status === 'published' ? '#166534' : '#854d0e',
                                        fontSize: '12px'
                                    }}>
                                        {item.publish_status}
                                    </span>
