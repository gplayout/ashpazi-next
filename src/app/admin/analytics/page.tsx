...
interface AnalyticsEvent {
    ...
    entity_id: string;
    created_at: string;
    metadata?: { sku?: string; [key: string]: unknown } | null;
    [key: string]: unknown;
}
...
async function getAnalyticsData(): Promise<AnalyticsData> {
    ...
    const whatsapp = (counts as AnalyticsEvent[]).filter(c => c.channel === 'whatsapp').length;
    const sms = (counts as AnalyticsEvent[]).filter(c => c.channel === 'sms').length;
    ...
    return { total, whatsapp, sms, feed: (feed as AnalyticsEvent[]) || [] };
}

// --- Components ---

function StatCard({ title, value, sub, icon: Icon, colorClass }: StatCardProps): React.JSX.Element {
    return (
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-start justify-between hover:shadow-md transition-shadow">
            ...
            <div
                className={`p-3 rounded-xl ${colorClass.replace('text-', 'bg-').replace('600', '100')}`}
            >
...
function FeedItem({ item }: FeedItemProps): React.JSX.Element {
    ...
    return (
        <div className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-0">
            <div className="flex items-center gap-4">
                <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center ${isWhatsapp ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'}`}
                >
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-gray-500 mt-1">
                            Real-time monitoring of marketplace conversions.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                            ...
                            <div>
                                <div className="flex justify-between text-sm font-medium mb-2">
                                    ...
                                    <span className="text-gray-900">
                                        {data.whatsapp} ({waPercent}%)
                                    </span>
                                </div>
                                <div className="h-4 w-full bg-gray-100 rounded-full overflow-hidden">
                                    <div
                                        style={{ width: `${waPercent}%` }}
                                        className="h-full bg-gradient-to-r from-green-400 to-green-600 rounded-full shadow-sm transition-all duration-1000 ease-out"
                                    ></div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="h-4 w-full bg-gray-100 rounded-full overflow-hidden">
                                    <div
                                        ...
                                        className="h-full bg-gradient-to-r from-blue-400 to-blue-600 rounded-full shadow-sm transition-all duration-1000 ease-out"
                                    ></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-8 pt-6 border-t border-gray-50">
                            ...
                            <p className="text-sm text-gray-500 leading-relaxed">
                                WhatsApp is performing significantly better than SMS, accounting for{' '}
                                <strong className="text-gray-900">{waPercent}%</strong> of all
                                conversion clicks. Consider optimizing the WhatsApp message copy to
                                further increase granular conversion.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[500px]">
                        ...
                        <div className="p-4 border-t border-gray-100 bg-gray-50/50 text-center">
                            <Link
                                href="#"
                                className="text-xs font-medium text-indigo-600 hover:text-indigo-800 flex items-center justify-center gap-1"
                            >
