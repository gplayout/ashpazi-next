...
export default function AdminReviewClient({ recipes }: AdminReviewClientProps): React.JSX.Element {
    ...
    async function handleMapExisting(rawLine: string | undefined): Promise<void> {
    ...
    async function handleCreateNew(rawLine: string | undefined): Promise<void> {
        const cleaned = cleanIngredientName(rawLine || '');
        ...
        <div style={{ display: 'flex', gap: '20px', padding: '20px' }}>
            ...
            <div style={{ width: '300px', borderRight: '1px solid #ccc' }}>
                ...
                <ul style={{ listStyle: 'none', padding: 0 }}>
                    {recipes.map(r => (
                        ...
                        >
                            <strong>#{r.legacy_recipe_id}</strong>
                            <br />
            ...
            <div style={{ flex: 1 }}>
                {selectedRecipe ? (
                    <div>
                        <h2>
                            Review: {selectedRecipe.recipes?.name} (#
                            {selectedRecipe.legacy_recipe_id})
                        </h2>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <button
                                onClick={() =>
                                    handleMapExisting(selectedRecipe.error_log?.raw_line)
                                }
                            >
                                Map to Existing
                            </button>
                            ...
                            <button
                                onClick={() => handleCreateNew(selectedRecipe.error_log?.raw_line)}
                            >
                                Create New
                            </button>
